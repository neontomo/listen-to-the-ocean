...
var amountBubbles = 9;
amountFish = ($(window).width() / (140 / 5)); // adjust amount to screen size
...
function randomID(len) {
    ...
    var type = 0;
	var len = (len) ? len : 20;
    var c = ['b', 'c', 'd', 'f', 'g', 'j', 'k', 'l', 'm', 'n', 'p', 'q', 's', 't', 'v', 'x', 'z', 'h', 'r', 'w', 'y'];
    var v = ['a','e','i','o','u'];
...
function updateScores() {
	if ($('.fish:not([dead])').length > 0) { // if fish are alive
...
function placeRandom(fish, direction) {
	...
	var left = (direction == 0) ? marginOutside * (-1) : ($(window).width() + marginOutside);
...
function generateFish(which) {
	...
	var which = (which === undefined || which === false) ? randomInteger(0, amountFishImages) : which;
...
function killFish(fish) {
	...
	fish.stop()
		.attr('dead', 'true')
		.animate({ 'top': $(window).height() - 50 }, { duration: speed, queue: false });
...
function createTrash() {
	...
	var rotate = 'rotate' + randomInteger(1, 2) + ' ' + randomInteger(3, 8) + 's linear infinite';
	...
	var trash = $('<div class="trash">')
				...
				.animate({ top: $(window).height() + 100, left: toLeft }, speed, function () {
					$(this).remove();
				});

	var trashImg = $('<img>')
					.attr('src', 'img/trash/' + which + '.png');
...
setInterval(function () { // flap the fish
...
setInterval(function () { // check collisions
	$('.trash').each(function () {
...
for (var i = 0; i < amountFish; i++) { // add fish
	generateFish();
}
